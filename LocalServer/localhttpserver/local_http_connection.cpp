...
void LocalHttpConnection::Open(boost::shared_ptr<tcp::socket> psock)
{
	...
	if (psock)
	{
		http_service_ = CHttpService::create(psock, shared_from_this());
		INFO_LOG(LOCAL_SERVER_LOG, _T("LocalHttpConnection::Open this=0x%08x http_service_ = 0x%08x\r\n"), shared_from_this().get(),http_service_.get());
...
void LocalHttpConnection::Close(void)
{
	...
	INFO_LOG(LOCAL_SERVER_LOG, _T("已调用 HTTP 关闭连接函数! this=0x%08x http_service_ = 0x%08x\r\n"), shared_from_this().get(),http_service_.get());

	is_open_ = false;

	if (http_service_.get())
	{
		INFO_LOG(LOCAL_SERVER_LOG, _T("LocalHttpConnection::Close(void) 已调用 http_service_ 关闭连接函数! this=0x%08x http_service_ = 0x%08x\r\n"), shared_from_this().get(),http_service_.get());
...
void LocalHttpConnection::on_service_close(void)
{
	...
	{
		INFO_LOG(LOCAL_SERVER_LOG, _T("on_service_close: %lld\n"), request_info_.call_id);
...
bool LocalHttpConnection::parse_http_request(std::string data)
{
	...
	INFO_LOG(LOCAL_SERVER_LOG, _T("request_info_.call_id %lld this 0x%x\r\n"), request_info_.call_id, shared_from_this().get());
...
//#endif
	...
	else if (request_info_.type.compare("download") == 0)
	{
		...
		INFO_LOG(LOCAL_SERVER_LOG, _T("创建离线下载任务, vid = %d, uid = %lld, start_pos=%d, call_id=%lld\r\n"), request_info_.vid, uid, request_info_.startpos, request_info_.call_id);
