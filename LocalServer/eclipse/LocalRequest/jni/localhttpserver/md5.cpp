...
static void MD5_transform(php_uint32[4], const unsigned char[64]);
...
#define FF(a, b, c, d, x, s, ac) { (a) += F ((b), (c), (d)) + (x) + (php_uint32)(ac); (a) = ROTATE_LEFT ((a), (s)); (a) += (b); }
#define GG(a, b, c, d, x, s, ac) { (a) += G ((b), (c), (d)) + (x) + (php_uint32)(ac); (a) = ROTATE_LEFT ((a), (s)); (a) += (b); }
#define HH(a, b, c, d, x, s, ac) { (a) += H ((b), (c), (d)) + (x) + (php_uint32)(ac); (a) = ROTATE_LEFT ((a), (s)); (a) += (b); }
#define II(a, b, c, d, x, s, ac) { (a) += I ((b), (c), (d)) + (x) + (php_uint32)(ac); (a) = ROTATE_LEFT ((a), (s)); (a) += (b); }
...
static void MD5_transform(php_uint32 state[4], const unsigned char block[64])
{
...
bool MD5_file_hash(char* outstr, char* filename)
{
	return true;
}
...
/* instr ÊäÈë×Ö·û´®£¬
 * outstr Êä³ö¼ÓÃÜºóµÄ×Ö·û´®
 */

void MD5_hash(char *outstr, char *instr)
{
...
std::string MD5_to_string( unsigned char  md5[16])
{
    ...
    while (i < 16)
    {
        unsigned int  num = (unsigned int)md5[i];
        ostr << setw(2) << setfill('0') << hex << num;
        i++;
    }
