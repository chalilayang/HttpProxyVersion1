...
void LocalHttpServer::on_connection_close(boost::shared_ptr<LocalHttpConnection> con, int call_id)
{
	if (false == is_running_) return;
...
void LocalHttpServer::on_connection_update(sh_int_64 uid, int call_id, boost::shared_ptr<LocalHttpConnection> sp_connection)
{
	if (false == is_running_) return;
	...
	//如果有相同的uid,要先关闭掉原来的connection
 	/*for (it = sock_connection_map_.begin(); it != sock_connection_map_.end();)
 	{
 		if (it->second.first == uid && it->first.get() != NULL && it->first.get() != sp_connection.get())
 		{
			INFO_LOG(LOCAL_SERVER_LOG, _T("删除已有的 connection,  uid=%lld, size=%d\r\n"), uid, sock_connection_map_.size());
	...
	for (it = sock_connection_map_.begin(); it != sock_connection_map_.end(); it++)
	{
		...
		{
			INFO_LOG(LOCAL_SERVER_LOG, _T("覆盖原有callid和uid,old_callid=%d, newcall_id=%d\r\n"), it->second.second, call_id);
			it->second.first = uid;
			it->second.second = call_id;
...
void LocalHttpServer::on_recv_p2p_video_data(sh_int_64 unique_id, sh_uint_32 index, sh_int_8_p data, sh_uint_32 len, bool isheader, sh_int_32 real_mp4_size, sh_int_32 cur_download_pos, int32_t call_id)
{
	if (!isheader && len <= 0)
	{
		int a = 0;
...
void LocalHttpServer::recv_p2p_video_data(sh_int_64 unique_id, sh_uint_32 index, std::string data, sh_uint_32 len, bool isheader, sh_int_32 real_mp4_size, sh_int_32 cur_download_pos, int32_t call_id)
{
	if (false == is_running_) return;

	if (unique_id < 0 || index < 0 || call_id < 0)
	{
		INFO_LOG(LOCAL_SERVER_LOG, _T("recv data happen error 异常!  uid = %lld, section = %d, call_id=%d\r\n"), unique_id, index, call_id);
	}
	...
	if (sp_connection.get() == NULL)
	{
		INFO_LOG(LOCAL_SERVER_LOG, _T("connection is NULL, uid=%lld, section=%d, pos=%d, call_id=%d\r\n"), unique_id, index, cur_download_pos, call_id);
		return;
	}
...
boost::shared_ptr<LocalHttpConnection> LocalHttpServer::get_http_connection(sh_int_64 unique_id, int call_id)
{
	boost::mutex::scoped_lock lock(mutex_);
	boost::shared_ptr<LocalHttpConnection> sp_connection;
	SHConnectionMap::iterator it;
...
sh_int_32 LocalHttpServer::get_call_id_num(void)
{
	boost::mutex::scoped_lock lock(call_id_mutex_);
	static sh_int_32 call_id = 0;
	if (++call_id <= 0)
	{
		call_id = 1;
	}
