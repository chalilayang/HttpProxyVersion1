...
//	m_close_timer->async_wait(boost::bind(&HttpService::close, shared_from_this()));
	close();
}
...
void HttpService::handle_write(const boost::system::error_code& ec, size_t trans_bytes, const IOBuffer& buf)
{
	...
	if (!ec)
	{
		if (cwiobuff_.size() == 0)
		{
			INFO_LOG(LOCAL_SERVER_LOG, _T("error cwiobuff_.size() == 0 socket: 0x%08x\n"), m_psocket.get());
		}
